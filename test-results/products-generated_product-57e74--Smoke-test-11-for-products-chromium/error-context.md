...
- Name: products\generated_products_test_11.spec.ts >> Products Module - Test Suite 11 >> Smoke test 11 for products
- Location: tests\e2e\products\generated_products_test_11.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Products Module - Test Suite 11', () => {
  5  |   test('Smoke test 11 for products', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 11 for products', async ({ adminPage }) => {
