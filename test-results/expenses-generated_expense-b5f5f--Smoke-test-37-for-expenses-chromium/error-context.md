...
- Name: expenses\generated_expenses_test_37.spec.ts >> Expenses Module - Test Suite 37 >> Smoke test 37 for expenses
- Location: tests\e2e\expenses\generated_expenses_test_37.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Expenses Module - Test Suite 37', () => {
  5  |   test('Smoke test 37 for expenses', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 37 for expenses', async ({ adminPage }) => {
