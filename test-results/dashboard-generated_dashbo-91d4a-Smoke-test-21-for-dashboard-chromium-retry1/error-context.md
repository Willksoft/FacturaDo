...
- Name: dashboard\generated_dashboard_test_21.spec.ts >> Dashboard Module - Test Suite 21 >> Smoke test 21 for dashboard
- Location: tests\e2e\dashboard\generated_dashboard_test_21.spec.ts:5:3
...
```yaml
- generic [ref=f1e4]:
  - img "FacturaDo" [ref=f1e5]
  - heading "FacturaDo" [level=1] [ref=f1e6]
  - paragraph [ref=f1e7]: Preparando tu espacio de trabajo...
```
...
```ts
  ...
  3  | 
  4  | test.describe('Dashboard Module - Test Suite 21', () => {
  5  |   test('Smoke test 21 for dashboard', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 21 for dashboard', async ({ adminPage }) => {
