...
- Name: dgii\generated_dgii_test_21.spec.ts >> Dgii Module - Test Suite 21 >> Smoke test 21 for dgii
- Location: tests\e2e\dgii\generated_dgii_test_21.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dgii Module - Test Suite 21', () => {
  5  |   test('Smoke test 21 for dgii', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 21 for dgii', async ({ adminPage }) => {
