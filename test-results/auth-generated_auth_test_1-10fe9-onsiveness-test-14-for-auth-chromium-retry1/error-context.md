...
- Name: auth\generated_auth_test_14.spec.ts >> Auth Module - Test Suite 14 >> UI responsiveness test 14 for auth
- Location: tests\e2e\auth\generated_auth_test_14.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 14', () => {
  5  |   test('Smoke test 14 for auth', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 14 for auth', async ({ adminPage }) => {
