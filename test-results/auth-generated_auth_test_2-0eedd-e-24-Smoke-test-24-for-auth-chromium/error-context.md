...
- Name: auth\generated_auth_test_24.spec.ts >> Auth Module - Test Suite 24 >> Smoke test 24 for auth
- Location: tests\e2e\auth\generated_auth_test_24.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 24', () => {
  5  |   test('Smoke test 24 for auth', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 24 for auth', async ({ adminPage }) => {
