...
- Name: invoices\generated_invoices_test_15.spec.ts >> Invoices Module - Test Suite 15 >> Smoke test 15 for invoices
- Location: tests\e2e\invoices\generated_invoices_test_15.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Invoices Module - Test Suite 15', () => {
  5  |   test('Smoke test 15 for invoices', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 15 for invoices', async ({ adminPage }) => {
