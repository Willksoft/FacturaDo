...
- Name: products\generated_products_test_23.spec.ts >> Products Module - Test Suite 23 >> Smoke test 23 for products
- Location: tests\e2e\products\generated_products_test_23.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Products Module - Test Suite 23', () => {
  5  |   test('Smoke test 23 for products', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 23 for products', async ({ adminPage }) => {
