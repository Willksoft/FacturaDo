...
- Name: dgii\generated_dgii_test_36.spec.ts >> Dgii Module - Test Suite 36 >> UI responsiveness test 36 for dgii
- Location: tests\e2e\dgii\generated_dgii_test_36.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dgii Module - Test Suite 36', () => {
  5  |   test('Smoke test 36 for dgii', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 36 for dgii', async ({ adminPage }) => {
