...
- Name: dashboard\generated_dashboard_test_28.spec.ts >> Dashboard Module - Test Suite 28 >> Smoke test 28 for dashboard
- Location: tests\e2e\dashboard\generated_dashboard_test_28.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dashboard Module - Test Suite 28', () => {
  5  |   test('Smoke test 28 for dashboard', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 28 for dashboard', async ({ adminPage }) => {
