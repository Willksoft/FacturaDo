...
- Name: dashboard\generated_dashboard_test_16.spec.ts >> Dashboard Module - Test Suite 16 >> Smoke test 16 for dashboard
- Location: tests\e2e\dashboard\generated_dashboard_test_16.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dashboard Module - Test Suite 16', () => {
  5  |   test('Smoke test 16 for dashboard', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 16 for dashboard', async ({ adminPage }) => {
