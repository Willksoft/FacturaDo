...
- Name: auth\generated_auth_test_27.spec.ts >> Auth Module - Test Suite 27 >> Smoke test 27 for auth
- Location: tests\e2e\auth\generated_auth_test_27.spec.ts:5:3
...
Call log:
  - Expect "toHaveURL" with timeout 5000ms
    11 × locator resolved to <html lang="es-DO">…</html>
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 27', () => {
  5  |   test('Smoke test 27 for auth', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 27 for auth', async ({ adminPage }) => {
