...
- Name: accounting\generated_accounting_test_29.spec.ts >> Accounting Module - Test Suite 29 >> Smoke test 29 for accounting
- Location: tests\e2e\accounting\generated_accounting_test_29.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Accounting Module - Test Suite 29', () => {
  5  |   test('Smoke test 29 for accounting', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 29 for accounting', async ({ adminPage }) => {
