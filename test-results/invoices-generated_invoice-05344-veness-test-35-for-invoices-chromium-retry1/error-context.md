...
- Name: invoices\generated_invoices_test_35.spec.ts >> Invoices Module - Test Suite 35 >> UI responsiveness test 35 for invoices
- Location: tests\e2e\invoices\generated_invoices_test_35.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Invoices Module - Test Suite 35', () => {
  5  |   test('Smoke test 35 for invoices', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 35 for invoices', async ({ adminPage }) => {
