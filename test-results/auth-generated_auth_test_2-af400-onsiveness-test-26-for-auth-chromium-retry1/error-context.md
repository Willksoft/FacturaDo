...
- Name: auth\generated_auth_test_26.spec.ts >> Auth Module - Test Suite 26 >> UI responsiveness test 26 for auth
- Location: tests\e2e\auth\generated_auth_test_26.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 26', () => {
  5  |   test('Smoke test 26 for auth', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 26 for auth', async ({ adminPage }) => {
