...
- Name: clients\generated_clients_test_17.spec.ts >> Clients Module - Test Suite 17 >> Smoke test 17 for clients
- Location: tests\e2e\clients\generated_clients_test_17.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Clients Module - Test Suite 17', () => {
  5  |   test('Smoke test 17 for clients', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 17 for clients', async ({ adminPage }) => {
