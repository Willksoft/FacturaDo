...
- Name: dgii\generated_dgii_test_33.spec.ts >> Dgii Module - Test Suite 33 >> UI responsiveness test 33 for dgii
- Location: tests\e2e\dgii\generated_dgii_test_33.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dgii Module - Test Suite 33', () => {
  5  |   test('Smoke test 33 for dgii', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 33 for dgii', async ({ adminPage }) => {
