...
- Name: auth\generated_auth_test_11.spec.ts >> Auth Module - Test Suite 11 >> UI responsiveness test 11 for auth
- Location: tests\e2e\auth\generated_auth_test_11.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 11', () => {
  5  |   test('Smoke test 11 for auth', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 11 for auth', async ({ adminPage }) => {
