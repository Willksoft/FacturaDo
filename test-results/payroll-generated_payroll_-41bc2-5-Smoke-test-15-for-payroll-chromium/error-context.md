...
- Name: payroll\generated_payroll_test_15.spec.ts >> Payroll Module - Test Suite 15 >> Smoke test 15 for payroll
- Location: tests\e2e\payroll\generated_payroll_test_15.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Payroll Module - Test Suite 15', () => {
  5  |   test('Smoke test 15 for payroll', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 15 for payroll', async ({ adminPage }) => {
