...
- Name: payroll\generated_payroll_test_32.spec.ts >> Payroll Module - Test Suite 32 >> Smoke test 32 for payroll
- Location: tests\e2e\payroll\generated_payroll_test_32.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Payroll Module - Test Suite 32', () => {
  5  |   test('Smoke test 32 for payroll', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 32 for payroll', async ({ adminPage }) => {
