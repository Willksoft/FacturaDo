...
- Name: settings\generated_settings_test_23.spec.ts >> Settings Module - Test Suite 23 >> Smoke test 23 for settings
- Location: tests\e2e\settings\generated_settings_test_23.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Settings Module - Test Suite 23', () => {
  5  |   test('Smoke test 23 for settings', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 23 for settings', async ({ adminPage }) => {
