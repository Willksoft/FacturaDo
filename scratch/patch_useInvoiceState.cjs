...
// 1. Mappings From Db
const mapFromTarget = `  category: db.category || undefined,
  createdAt: db.created_at
});`;
const mapFromReplacement = `  category: db.category || undefined,
  createdAt: db.created_at,
  isKit: db.is_kit || false,
  kitItems: db.kit_items || [],
  batches: db.batches || [],
  stockLevels: db.stock_levels || []
});`;
if(content.includes(mapFromTarget)) content = content.replace(mapFromTarget, mapFromReplacement);

// 2. Mappings To Db
const mapToTarget = `  category: prod.category || null,
  created_at: prod.createdAt,
  is_deleted: false
});`;
const mapToReplacement = `  category: prod.category || null,
  created_at: prod.createdAt,
  is_kit: prod.isKit || false,
  kit_items: prod.kitItems || [],
  batches: prod.batches || [],
  stock_levels: prod.stockLevels || [],
  is_deleted: false
});`;
if(content.includes(mapToTarget)) content = content.replace(mapToTarget, mapToReplacement);

// 3. Logic Deduction
const logicTarget = `    if (!isQuote) {
...
const logicReplacement = `    if (!isQuote) {
...
if(content.includes(logicTarget)) {
    ...
    console.log('useInvoiceState.ts patched successfully!');
} else {
    console.error('Logic target not found in useInvoiceState.ts!');
}
