...
const file = path.join(__dirname, '../src/features/billing/InvoiceCreator.tsx');
...
const addReplacement = `        discount: currentLineDiscount,
        ...
        showImage: templateSettings?.showProductPhotos !== false, // default true if globally enabled
      };
      setItems([...items, newItem]);`;
content = content.replace(addTarget, addReplacement);

// 2. Add an inline toggle or checkbox to the item row for showImage
// Let's add a small photo thumbnail and a toggle next to the product name if it has an image.
...
console.log('InvoiceCreator.tsx patched with product image thumbnail toggle');
