...
export default function DashboardSetupGuide({
  ...
  useEffect(() => {
    const isHidden = localStorage.getItem('facturado_hide_setup_guide_v3');
    if (isHidden !== 'true') {
      setIsVisible(true);
    }
  }, []);
  ...
  const handleDismiss = () => {
    localStorage.setItem('facturado_hide_setup_guide_v3', 'true');
    setIsVisible(false);
    setShowModal(false);
  };
      ...
      <div className="fixed bottom-40 md:bottom-24 right-6 z-40 flex flex-col items-end animate-fade-in">
        ...
        >
          ...
          <div className="flex flex-col items-start justify-center">
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider leading-none">Tu Progreso</span>
            <span className="text-xs font-medium text-neutral-200 mt-0.5">{completedSteps}/{totalSteps} Pasos Completados</span>
          </div>
              ...
              <div className="flex flex-col md:flex-row h-full overflow-hidden">
                ...
                <div className="bg-[#1A2732] text-white p-8 md:w-[320px] flex flex-col shrink-0 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/10 text-white text-[11px] font-bold uppercase rounded-full tracking-wider mb-6 border border-white/10">
                      <Sparkles className="w-3.5 h-3.5 text-emerald-400" /> Domina FacturaDo
                    </div>
                    <h2 className="text-2xl font-bold mb-4 tracking-tight">Desbloquea todo el potencial</h2>
                    <p className="text-sm text-neutral-300 leading-relaxed mb-8">
                      Completa esta lista de tareas fundamentales para configurar tu negocio y aprovechar todas las herramientas del sistema.
                    </p>
                  </div>
                ...
                <div className="p-8 md:flex-1 overflow-y-auto bg-neutral-50 scrollbar-thin">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button 
                      ...
                      className="text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-4 py-1.5 rounded-full transition-colors"
                    >
                      ...
                      <div 
                        ...
                        className={`flex flex-col sm:flex-row sm:items-center gap-4 p-5 rounded-2xl border transition-all ${
                          step.isCompleted 
                            ...
                            : 'bg-white border-neutral-200 shadow-xs hover:border-indigo-300 hover:shadow-md relative overflow-hidden'
                        }`}
                      >
                        {!step.isCompleted && (
                        ...
                        <div className="flex items-start sm:items-center gap-4 flex-1">
                          <div className={`shrink-0 ${step.isCompleted ? 'text-emerald-500' : 'text-indigo-500 bg-indigo-50 p-2.5 rounded-xl'}`}>
                            {step.isCompleted ? (
                        ...
                        {!step.isCompleted && (
                          <div className="mt-3 sm:mt-0 sm:ml-auto shrink-0 flex items-center justify-end">
                            <Button
                              className="bg-[#1A2732] hover:bg-neutral-800 text-white text-xs uppercase font-bold tracking-wider h-9 px-4 rounded-xl shadow-xs w-full sm:w-auto"
                              onClick={step.action}
                            >
                              {step.actionLabel}
                            </Button>
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      Descartar Guía Definitivamente
                    </button>
                  </div>
                </div>
              </div>
