...
- Name: clients\generated_clients_test_15.spec.ts >> Clients Module - Test Suite 15 >> Smoke test 15 for clients
- Location: tests\e2e\clients\generated_clients_test_15.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Clients Module - Test Suite 15', () => {
  5  |   test('Smoke test 15 for clients', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 15 for clients', async ({ adminPage }) => {
