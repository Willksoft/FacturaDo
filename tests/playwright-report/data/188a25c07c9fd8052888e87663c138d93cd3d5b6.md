...
- Name: settings\generated_settings_test_10.spec.ts >> Settings Module - Test Suite 10 >> UI responsiveness test 10 for settings
- Location: tests\e2e\settings\generated_settings_test_10.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Settings Module - Test Suite 10', () => {
  5  |   test('Smoke test 10 for settings', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 10 for settings', async ({ adminPage }) => {
