...
- Name: dashboard\generated_dashboard_test_44.spec.ts >> Dashboard Module - Test Suite 44 >> Smoke test 44 for dashboard
- Location: tests\e2e\dashboard\generated_dashboard_test_44.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dashboard Module - Test Suite 44', () => {
  5  |   test('Smoke test 44 for dashboard', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 44 for dashboard', async ({ adminPage }) => {
