...
- Name: invoices\generated_invoices_test_3.spec.ts >> Invoices Module - Test Suite 3 >> Smoke test 3 for invoices
- Location: tests\e2e\invoices\generated_invoices_test_3.spec.ts:5:3
...
Call log:
  - Expect "toHaveURL" with timeout 5000ms
    13 × locator resolved to <html lang="es-DO">…</html>
...
```ts
  ...
  3  | 
  4  | test.describe('Invoices Module - Test Suite 3', () => {
  5  |   test('Smoke test 3 for invoices', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 3 for invoices', async ({ adminPage }) => {
