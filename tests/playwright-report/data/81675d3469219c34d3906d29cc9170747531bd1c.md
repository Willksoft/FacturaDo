...
- Name: auth\generated_auth_test_28.spec.ts >> Auth Module - Test Suite 28 >> UI responsiveness test 28 for auth
- Location: tests\e2e\auth\generated_auth_test_28.spec.ts:17:3
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 28', () => {
  5  |   test('Smoke test 28 for auth', async ({ adminPage }) => {
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 28 for auth', async ({ adminPage }) => {
