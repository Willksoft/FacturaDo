...
- Name: pos\generated_pos_test_3.spec.ts >> Pos Module - Test Suite 3 >> Smoke test 3 for pos
- Location: tests\e2e\pos\generated_pos_test_3.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Pos Module - Test Suite 3', () => {
  5  |   test('Smoke test 3 for pos', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 3 for pos', async ({ adminPage }) => {
