...
- Name: clients\generated_clients_test_24.spec.ts >> Clients Module - Test Suite 24 >> Smoke test 24 for clients
- Location: tests\e2e\clients\generated_clients_test_24.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Clients Module - Test Suite 24', () => {
  5  |   test('Smoke test 24 for clients', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 24 for clients', async ({ adminPage }) => {
