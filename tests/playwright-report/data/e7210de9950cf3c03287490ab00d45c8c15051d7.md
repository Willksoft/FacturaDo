...
- Name: invoices\generated_invoices_test_4.spec.ts >> Invoices Module - Test Suite 4 >> Smoke test 4 for invoices
- Location: tests\e2e\invoices\generated_invoices_test_4.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Invoices Module - Test Suite 4', () => {
  5  |   test('Smoke test 4 for invoices', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 4 for invoices', async ({ adminPage }) => {
