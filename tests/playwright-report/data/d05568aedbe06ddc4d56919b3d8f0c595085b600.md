...
- Name: products\generated_products_test_19.spec.ts >> Products Module - Test Suite 19 >> Smoke test 19 for products
- Location: tests\e2e\products\generated_products_test_19.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Products Module - Test Suite 19', () => {
  5  |   test('Smoke test 19 for products', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 19 for products', async ({ adminPage }) => {
