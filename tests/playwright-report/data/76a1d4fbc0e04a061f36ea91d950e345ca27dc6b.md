...
- Name: accounting\generated_accounting_test_28.spec.ts >> Accounting Module - Test Suite 28 >> Smoke test 28 for accounting
- Location: tests\e2e\accounting\generated_accounting_test_28.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Accounting Module - Test Suite 28', () => {
  5  |   test('Smoke test 28 for accounting', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 28 for accounting', async ({ adminPage }) => {
