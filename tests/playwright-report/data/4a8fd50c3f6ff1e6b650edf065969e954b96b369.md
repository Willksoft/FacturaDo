...
- Name: auth\generated_auth_test_13.spec.ts >> Auth Module - Test Suite 13 >> Smoke test 13 for auth
- Location: tests\e2e\auth\generated_auth_test_13.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Auth Module - Test Suite 13', () => {
  5  |   test('Smoke test 13 for auth', async ({ adminPage }) => {
...
> 7  |     await adminPage.goto('/dashboard');
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 13 for auth', async ({ adminPage }) => {
