...
- Name: dgii\generated_dgii_test_12.spec.ts >> Dgii Module - Test Suite 12 >> Smoke test 12 for dgii
- Location: tests\e2e\dgii\generated_dgii_test_12.spec.ts:5:3
...
```ts
  ...
  3  | 
  4  | test.describe('Dgii Module - Test Suite 12', () => {
  5  |   test('Smoke test 12 for dgii', async ({ adminPage }) => {
...
> 8  |     await expect(adminPage).toHaveURL(/.*dashboard/);
  ...
  15 |   });
  16 | 
  17 |   test('UI responsiveness test 12 for dgii', async ({ adminPage }) => {
